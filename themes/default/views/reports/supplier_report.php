...
<div class="tab-content">
	...
	<div id="purchase_order-con" class="tab-pane fade in">
		<?php 
			$v = "&supplier=" . $user_id;
			$v .= "&biller_id=" . $biller_id;
			if ($this->input->post('submit_purchase_order_report')) {
				if ($this->input->post('biller')) {
					$v .= "&biller=" . $this->input->post('biller');
				}
				if ($this->input->post('warehouse')) {
		...
		$(document).ready(function () {
			var oTable = $('#PurRData').dataTable({
				...
				"iDisplayLength": <?= $Settings->rows_per_page ?>,
				'bProcessing': true, 'bServerSide': true,
				'sAjaxSource': '<?=site_url('reports/getPurchaseOrder/?v=1' . $v)?>',
				'fnServerData': function (sSource, aoData, fnCallback) {
					aoData.push({
						"name": "<?= $this->security->get_csrf_token_name() ?>",
						"value": "<?= $this->security->get_csrf_hash() ?>"
        ...
        $(document).ready(function () {
            $('#form_po').hide();
            $('.poclosetoggle').click(function () {
                $("#form_po").slideDown();
                return false;
            });
            $('.potoggle').click(function () {
                $("#form_po").slideUp();
                return false;
            });
        });
				...
				<div class="box-icon">
					<ul class="btn-tasks">						
						<div class="box-icon">
							<ul class="btn-tasks">
								<li class="dropdown">
									<a href="#" class="potoggle tip" title="<?= lang('hide_form') ?>">
										<i class="icon fa fa-toggle-up"></i>
									</a>
								</li>
								<li class="dropdown">
									<a href="#" class="poclosetoggle tip" title="<?= lang('show_form') ?>">
										<i class="icon fa fa-toggle-down"></i>
									</a>
								</li>
							</ul>
			...
			<div class="box-content">
				<div class="row">
					<div class="col-lg-12">
						...
						<div id="form_po">                            
							<?php echo form_open("reports/supplier_report/" . $user_id.  "/#purchase_order-con"); ?>
                            <div class="row">
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        <label class="control-label" for="user"><?= lang("created_by"); ?></label>
                            ...
                            <div class="form-group">
                                <div
                                class="controls"> <?php echo form_submit('submit_purchase_order_report', $this->lang->line("submit"), 'class="btn btn-primary"'); ?> </div>
                            </div>
                            <?php echo form_close(); ?>
                        </div>
                        <div class="clearfix"></div>
	...
	<div id="top-products-con" class="tab-pane fade in">
		<?php
			...
			if ($this->input->post('biller')) {
                $v4 .= "&biller=" . $this->input->post('biller');
            }
