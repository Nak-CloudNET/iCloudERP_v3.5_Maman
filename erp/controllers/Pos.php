...
class Pos extends MY_Controller
{
    ...
    function getProductDataByCode($code = NULL, $warehouse_id = NULL, $cust_id = null, $suspend_id = null, $item_rows = null, $sub_total = null)
    {
        ...
        if($row){
           ...
           if ($opt->price != 0) {
			...
			$row->price_id = isset($group_prices[0]->id);
	...
	function add_payment($id = NULL)
    {
        ...
        $this->form_validation->set_rules('temp_reference_no', lang("reference_no"), 'trim|required|is_unique[payments.reference_no]');
        ...
        if ($this->form_validation->run() == true) {
			...
			if($paid_by == "deposit"){
				...
				$payment_reference = (($paid_by == 'deposit') ? $this->site->getReference('pay',$biller_id):($this->input->post('temp_reference_no') ? $this->input->post('temp_reference_no') : $this->site->getReference('sp',$biller_id)));
			}
