        ...
        if ($rows) {
            foreach ($rows as $row) {
				...
				if ($row->tax_rate) {
					...
					$pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => isset($combo_items), 'tax_rate' => isset($tax_rate), 'options' => isset($options),'expdates'=>isset($expdates),'group_prices'=>isset($group_prices), 'all_group_prices' => isset($all_group_prices), 'makeup_cost'=>isset($customer_group->makeup_cost), 'customer_percent' => isset($customer_percent), 'makeup_cost_percent'=>isset($percent->percent));
	...
	function add_new_delivery()
	{
        ...
        if ($this->form_validation->run('sales/add_new_delivery') == true)
        {
			...
			if($delivery){
                ...
                optimizeSystem(date('Y-m-d', strtotime($date)));
				if($delivery_id > 0){
