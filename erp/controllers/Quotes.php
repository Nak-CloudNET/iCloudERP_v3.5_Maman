        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                if ($row->tax_rate) {
					...
					$pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options,'warehouse_id'=>$warehouse_id,'customer_id'=>$customer_id,'group_prices'=>$group_prices, 'all_group_prices' => $all_group_prices, 'dropdown_group_prices' => $dropdown_group_prices, 'makeup_cost'=>$customer_group->makeup_cost, 'customer_percent' => $customer_percent,'makeup_cost_percent'=>(isset($percent->percent)?$percent->percent:0));
