...
class Pos_model extends CI_Model
{
        ...
        if ($this->db->insert('sales', $data)) {
			...
			if(strpos($data['paid'], '-') !== false){
				...
				$returns = array(
					'date' => $data['date'],
					'sale_id' => $sale_id,
					'reference_no' => $this->site->getReference('re', $data['biller_id']),
				...
				if ($this->db->insert('return_sales', $returns)) {
					...
					if ($this->site->getReference('re', $data['biller_id']) == $returns['reference_no']) {
					...
					if (!empty($payments)) {
						foreach($payments as $payment_return){
							...
							if ($this->site->getReference('sp', $data['biller_id']) == $returns['reference_no']) {
						...
						foreach ($payments as $payment) {
							if (!empty($payment) && isset($payment['amount']) && $payment['amount'] != 0) {
                                ...
                                if($payment['paid_by'] == 'Voucher'){
                                    if ($this->site->getReference('sp', $data['biller_id']) == $payment['reference_no']) {
                                        $this->site->updateReference('sp', $data['biller_id']);
                                    }
                                }
							}
    ...
    {
        $this->db->select('sales.*, users.username,erp_tax_rates.name AS tax,erp_payments.paid_by,erp_users.phone,erp_payments.cheque_no,erp_payments.cc_no,erp_payments.cc_type,erp_warehouses.name AS ware, erp_payments.pos_balance, erp_payments.pos_paid_other_rate,user2.username AS customer_name, erp_companies.award_points');
