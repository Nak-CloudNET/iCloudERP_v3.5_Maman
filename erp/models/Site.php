...
class Site extends CI_Model
{
	...
	public function getReference($field, $biller = null) {
		...
		if($this->get_setting()->separate_ref == 1){
			$biller_prefix = true;
			$biller_id = json_encode($biller);
			if($biller){
				$condition = array('DATE_FORMAT(date,"%Y-%m")' => date('Y-m'),'biller_id'=>$biller_id[0]);
			}

			if ($this->Settings->reference_format == 4 || $this->Settings->reference_format == 5) {
				$q = $this->db->get_where('order_ref', array('biller_id'=> $biller_id[0]), 1);
			} elseif($this->Settings->reference_format == 2) {
			    $this->db
                     ->select('order_ref.*')
                     ->from('order_ref')
                     ->where('DATE_FORMAT(date,"%Y")', date('Y'))
                    ->where_in('biller_id', JSON_decode($biller_id)[0])
                     ->limit(1);
				$q = $this->db->get();
			} else {
        ...
        if ($q->num_rows() > 0) {
            ...
            switch ($field) {
                case 'so':
					if($biller_prefix){
						if ($this->Settings->reference_format == 2) {
							$biller_prefix = $this->getBillerPrefix($biller)->biller_prefix;
							$prefix = $biller_prefix;
							break;
						} else {
				...
				case 'pp':
					if($biller_prefix){
						if ($this->Settings->reference_format == 2) {
							if(isset($this->getBillerPrefix($biller)->biller_prefix))$biller_prefix = $this->getBillerPrefix($biller)->biller_prefix;
							$prefix = $biller_prefix;
							break;
						} else {
		...
		if($this->get_setting()->separate_ref == 1){
			...
			if($biller){
				...
				if ($this->get_setting()->reference_format == 4 || $this->get_setting()->reference_format == 5) {
					...
					$q = $this->db->get_where('order_ref', array('DATE_FORMAT(date,"%Y")' => date('Y'), 'biller_id'=>$biller), 1);
					if ($q->num_rows() > 0) {
						$ref = $q->row();
	...
	public function calculateAVCost($product_id, $warehouse_id, $net_unit_price, $unit_price, $quantity, $product_name, $option_id, $item_quantity, $transaction_type, $transaction_id, $status,$expiry, $old_sqty) {
...
//						echo 'hello'; exit();
	...
	{
        $biller_id = json_encode($id);
        $id = JSON_decode($biller_id)[0];
		$this->db->select('*');
		$this->db->from("erp_companies");
		$this->db->where_in('id',$id);
		$q = $this->db->get();
