...
class Sales_model extends CI_Model
{
	...
	public function getProductNumber($term, $warehouse_id, $standard, $combo, $digital, $service, $category, $limit = 100)
    {
		if(preg_match('/\s/', $term))
		{
			$name = explode(" ", $term);
			$first = $name[0];
			$this->db->select('products.id, code, name, type, cost,warehouses_products.quantity, price,discount_by_item, tax_rate, tax_method, cf1, product_details,subcategory_id, details,COALESCE((SELECT GROUP_CONCAT(sp.`serial_number`) 
			...
			if(strpos($data['paid'], '-') !== true){
				if ($data['payment_status'] == 'partial' || $data['payment_status'] == 'paid' && !empty($payment)) {
					...
					if($payment['paid_by'] == 'deposit'){
                        ...
                        $deposit_balance = $deposit ? $deposit->deposit_amount : 0;
   ...
   public function updatePurchaseItem($id, $qty, $sale_item_id, $product_id = NULL, $warehouse_id = NULL, $option_id = NULL, $return_item_id = NULL, $return_date = NULL)
    ...
    public function addPayment($data = array())
    {
        ...
        if ($this->db->insert('payments', $data)) {
			$payment_id = $this->db->insert_id();
			if ($this->site->getReference('sp', json_decode($data['biller_id'])) == $data['reference_no']) {
				$this->site->updateReference('sp', $data['biller_id']);
			}
